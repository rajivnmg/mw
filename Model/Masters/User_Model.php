...
{
		$logger = Logger::getLogger('User Module');
	    $Query = "SELECT * FROM user_mast WHERE USER_TYPE != 'A'"; 
		$logger->debug($Query); // function to write log file
		$Result = DBConnection::SelectQuery($Query);
		return $Result;
}
...
// function to update user password from admin added on 7th July 2018
public static function UpdateUserPassByAdmin($USERID,$newpass)
{
	$logger = Logger::getLogger('User Module To update password by admin');
	    $Query="UPDATE user_mast SET PASSWD='".md5($newpass)."' WHERE USERID='$USERID'";    
		$logger->debug($Query); // function to write log file
		$Result = DBConnection::UpdateQuery($Query);		
		if($Result == QueryResponse::SUCCESS){
