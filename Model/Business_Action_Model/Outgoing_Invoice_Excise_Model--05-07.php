...
class Outgoing_Invoice_Excise_Model
{
    ...
    public static function createNewExciseInvoiceNumber($exciseStr){
        //$Query = "SELECT oinvoice_exciseID FROM outgoinginvoice_excise ORDER BY oinvoice_exciseID DESC LIMIT 1";
         if(MANESAR){
			$Query = "SELECT CONCAT('M/E-',((SUBSTR(oinvoice_No,5,10))+1)) AS new_outgoingInvoice FROM outgoinginvoice_excise WHERE oinvoice_No LIKE ('$exciseStr') ORDER BY oinvoice_exciseID DESC LIMIT 1";
   ...
   public static function GetLastInvoiceNumber(){
	        ...
	        if($totExciseInvoiceNumber>0){
	         ...
	         $s = sprintf("%06d", $firstExciseInvoiceNumber);
				if(MANESAR){
					return "M/E-".$FinancialYear.$s;
				}else{
					return "E-".$FinancialYear.$s;
				}
	        }
   }


    public static function InsertOutgoingInvoiceExcise($oinvoice_No,$pono, $BuyerID, $principalID,  $supplierID,  $mode_delivery,  $oinv_date, $oinv_time,  $dispatch_time,  $po_date,  $Supplier_stage,  $discount,  $inclusive_ed_tag, $freight_percent,  $freight_amount,  $pf_chrg,  $incidental_chrg,  $bill_value,  $payment_status, $remarks,  $userId,$ms,$ins_charge,$othc_charge,$_vehcle_no,$_supply_place,$_reverse_charge_payable,$_dnt_supply,$_docket_no){
        $date = date("Y-m-d");
        ...
        {	
            ...
            $Query = "INSERT INTO outgoinginvoice_excise (oinvoice_No,pono, BuyerID, principalID, supplierID, mode_delivery, oinv_date, oinv_time,dispatch_time, po_date, Supplier_stage, discount, inclusive_ed_tag, freight_percent, freight_amount, pf_chrg, incidental_chrg, bill_value, balanceAmount, payment_status, msid ,remarks, userId, insertDate,insurance_charge,other_charge,vehcle_no,place_of_supply,reverse_charge_payable,datetime_of_supply,docket_no) VALUES ('$oinvoice_No',$pono,  $BuyerID,  $principalID,  $supplierID,  '$mode_delivery',  '$date','$oinv_time',  '$dispatch_time',  $pono,  '$Supplier_stage',  $discount, '$inclusive_ed_tag', $freight_percent,  $freight_amount,  $pf_chrg,  $incidental_chrg,  $bill_value, $bill_value, '$payment_status','$ms','$remarks', '$userId', '$date', $ins_charge, $othc_charge,'$_vehcle_no','$_supply_place',$_reverse_charge_payable,'$_dnt_supply','$_docket_no')";
            $Result = DBConnection::InsertQuery($Query);
    ...
    public static function UpdateOutgoingInvoiceExcise($oinvoice_exciseID,$oinvoice_No,$pono,$BuyerID,$principalID,  $supplierID,  $mode_delivery,  $oinv_date, $oinv_time,  $dispatch_time,  $po_date,  $Supplier_stage,  $discount,  $total_ed,  $inclusive_ed_tag, $freight_percent,  $freight_amount,  $saleTax,  $pf_chrg,  $incidental_chrg,  $bill_value,  $payment_status, $remarks,  $userId,$ms){
		...
		$Query = $Query."mode_delivery = '$mode_delivery', oinv_date = '$oinv_date1', oinv_time = '$oinv_time', dispatch_time = '$dispatch_time',";
		$Query = $Query."po_date = $po_date, Supplier_stage = '$Supplier_stage', discount = $discount, total_ed = $total_ed,";
		$Query = $Query."inclusive_ed_tag = '$inclusive_ed_tag', freight_percent = $freight_percent, freight_amount = $freight_amount,";
		$Query = $Query."saleTax = $saleTax, pf_chrg = $pf_chrg, incidental_chrg = $incidental_chrg, bill_value = $bill_value,balanceAmount = $bill_value, ";
		$Query = $Query."payment_status = '$payment_status',msid='$ms', remarks = '$remarks', userId = '$userId' ";
...
//         }
    ...
    public static function GetOutgoingInvoiceExciseDetails($oinvoice_exciseID){
        $Query = "SELECT oie.*,bm.BuyerName,pm.Principal_Supplier_Name as PRINCIPALNAME,sm.Principal_Supplier_Name as SUPPLIERNAME,pod.bpoType,pod.pf_chrg AS popf_charge,pod.incidental_chrg AS poinc_charge
		FROM outgoinginvoice_excise as oie LEFT JOIN buyer_master as bm ON oie.BuyerID = bm.BuyerId LEFT JOIN principal_supplier_master as pm ON oie.principalID = pm.Principal_Supplier_Id LEFT JOIN principal_supplier_master as sm ON oie.supplierID = sm.Principal_Supplier_Name,purchaseorder AS pod WHERE pod.bpoId=oie.pono AND oie.oinvoice_exciseID = $oinvoice_exciseID";
...
class Outgoing_Invoice_Excise_Model_Details
{
	...
	{	
		...
		while ($Row = mysql_fetch_array($result, MYSQL_ASSOC)) {
           ...
           $tot_issue_qty=self::GetPoIssueQty($bpod_Id,$_item_id,$bpoType,'E');
