...
class Search_Model
{
 ...
 public static function getItemStockQty($bpoId){
		...
		$colr = 'R';
		//$Query="SELECT pod.po_codePartNo, pod.po_ed_applicability, SUM(pod.po_qty ) AS Total FROM purchaseorder_detail AS pod WHERE pod.`bpoId` = '$bpoId' AND pod.po_item_stage !='OIG' AND pod.po_item_stage !='POIG' GROUP BY po_codePartNo";
		
		$TotalPo = 0;
		$QueryPo="SELECT SUM(pod.po_qty ) AS TotalPo FROM purchaseorder_detail AS pod WHERE pod.`bpoId` = '$bpoId' ";
		$ResultPo = DBConnection::SelectQuery($QueryPo);
		if(mysql_num_rows($ResultPo) > 0){
			$RowPo = mysql_fetch_array($ResultPo, MYSQL_ASSOC);
			$TotalPo=$RowOut['TotalPo'];			
		}
		
		// added on 25 August 2018 to Get Total outgoing quentity in invoice 
		$QueryOut="SELECT oe.pono, SUM(oied.issued_qty) AS TotalIssued FROM outgoinginvoice_excise_detail AS oied INNER JOIN outgoinginvoice_excise as oe ON oe.oinvoice_exciseID = oied.oinvoice_exciseID WHERE oe.`pono` = '$bpoId'";
		$TotalIssued = 0;
		//echo '<br/>Query --> '.$QueryOut; die;
		$ResultOut = DBConnection::SelectQuery($QueryOut);
		if(mysql_num_rows($ResultOut) > 0){
			$RowOut = mysql_fetch_array($ResultOut, MYSQL_ASSOC);
			$TotalIssued=$RowOut['TotalIssued'];			
		}
		
		if($TotalPo == $TotalIssued){
			return $colr ="B"; break;
		}
