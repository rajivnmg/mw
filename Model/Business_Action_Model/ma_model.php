...
SELECT BILL_VALUE AS CNT FROM outgoinginvoice_nonexcise AS ONE,purchaseorder AS PO WHERE ONE.BUYERID=PO.BPOID AND PO.BUYERID=$BUYERID
	...
	public static function getItemDetails($itemId,$principalId,$type){		
		//commented on 25 August 2018 due to wrong data calculation on landing price			
		$Query = "SELECT im.Item_Code_Partno FROM item_master as im WHERE ItemId=$itemId LIMIT 1";
		$Result = DBConnection::SelectQuery($Query);
		if(mysql_num_rows($Result) > 0){
			$Row = mysql_fetch_array($Result, MYSQL_ASSOC);
			return $Row['Item_Code_Partno'];	
			exit;
		}else{	
			$p = null;
			return $p;
	...
	public static function getItemLandingPrice($itemId,$principalId,$type){
		...
		if($type =="E"){
				...
				$Query2 = "SELECT ied.entryDId as id,IFNULL(MAX(ied.landing_price),0) as price,IFNULL(ie.pf_chrg,0) as pf_charge,IFNULL(ie.insurance,0) as insurance,IFNULL(ie.freight,0) as freight,ied.basic_purchase_price,ied.discount_percent,ied.packing_percent,ied.insurance_percent,ied.freight_percent,ied.other_percent FROM incominginvoice_excise_detail as ied 
				INNER JOIN incominginvoice_excise as ie on ie.entryId = ied.entryId WHERE ied.itemID_code_partNo=$itemId AND ie.principalId = $principalId ";
				...
				if(mysql_num_rows($Result) > 0){
					...
					$packing_percent = $Row['packing_percent'];
					$insurance_percent = $Row['insurance_percent'];
					$freight_percent = $Row['freight_percent'];
					$other_percent = $Row['other_percent'];					
					$total_bill_val = $Row['basic_purchase_price'];	
					
					$discount = (($bsp * $discount_percent)/100);
					$packing = (($bsp * $packing_percent)/100);
					$insurance = (($bsp * $insurance_percent)/100);
					$freight = (($bsp * $freight_percent)/100);
					$other = (($bsp * $other_percent)/100);					
					//~ $pf_val =  $total_bill_val/$pf_charge;
					//~ $inc_val =  $total_bill_val/$insurance;
					//~ $frt_val =  $total_bill_val/$freight;					
					$actual_land = ($bsp - ($discount) + ($packing + $insurance + $freight + $other));
					return $actual_land;
					exit;
				}else{	
					$p = 0;
