...
class Principal_Supplier_Master_Model
{
	...
	public function __construct($principalsupplierid, $principalsuppliername, $add1, $add2, $cityid, $stateid, $pincode, $pcrange,$pcdivision, $commissionrate, $ecccodeno, $tinno, $panno, $type, $tax_type, $bankname, $accountnumber, $bankaddress, $rtgs, $neft,$accounttype, $cityname, $statename, $_EmployeeList, $_GSTList,$PROFIT_MARGIN){
		...
		$this->PROFIT_MARGIN = $PROFIT_MARGIN; // Added to set profit margin for Principal on 23-07-2018
	}
	...
	{	
		...
		while ($Row = mysql_fetch_array($result, MYSQL_ASSOC)) {
           ...
           $_principal_supplier_id = $Row['Principal_Supplier_Id'];
			...
			$PROFIT_MARGIN = $Row['PROFIT_MARGIN']; // Added by RK
            if($Principal_supplier_id > 0)
            {
			...
			$newObj = new Principal_Supplier_Master_Model($_principal_supplier_id, $_principal_supplier_name, $_add1, $_add2,$_city_id, $_state_id, $_pincode, $_pc_range, $_pc_division, $_commission_rate, $_ecc_codeno, $_tin_no, $_pan_no, $_type, $_tax_type, $_bankname, $_accountnumber, $_bankaddress, $_rtgs, $_neft, $_accounttype, $_city_name, $_state_name, $_EmployeeList, $_GSTList,$PROFIT_MARGIN);
	...
	public static function Insert_Principal_Supplier($Principal_Supplier_Name, $ADD1, $ADD2, $CITYID, $STATEID, $PINCODE, $PS_RANGE,$PS_DIVISION, $PS_COMMISSIONERATE, $ECC_CODENO, $TINNO, $PANNO, $TYPE, $TAX_TYPE, $UserId,$PROFIT_MARGIN){
	...
	public static function Update_Principal_Supplier($PRINCIPAL_SUPPLIER_ID,$Principal_Supplier_Name, $ADD1, $ADD2, $PINCODE, $PS_RANGE,$PS_DIVISION, $PS_COMMISSIONERATE, $ECC_CODENO, $TINNO, $PANNO,$CITYID, $STATEID, $TAX_TYPE,$PROFIT_MARGIN){
	/* EOF to add to add GST details for Principal by Ayush Giri on 21-06-2017 */
		...
		*/ 
		...
		$Query = "UPDATE principal_supplier_master SET Principal_Supplier_Name = '".$Principal_Supplier_Name."', ADD1 = '".$ADD1."', ADD2 = '".$ADD2."',CITYID = '".$CITYID."', STATEID = '".$STATEID."', PINCODE = '".$PINCODE."',PS_RANGE = '".$PS_RANGE."',PS_DIVISION = '".$PS_DIVISION."', PS_COMMISSIONERATE = '".$PS_COMMISSIONERATE."', ECC_CODENO = '".$ECC_CODENO."', TINNO = '".$TINNO."', PANNO = '".$PANNO."', Tax_Type = '".$TAX_TYPE."', PROFIT_MARGIN = '".$PROFIT_MARGIN."' WHERE Principal_Supplier_Id = $PRINCIPAL_SUPPLIER_ID";
		/* EOF to add to add GST details for Principal by Ayush Giri on 21-06-2017 */
...
//echo $Query;exit;
    ...
    // GET PRINCIPAL SUPPLIER PROFIT MARGIN %
     public static function Get_principal_supplier_profitMargin($principlaaId)
    {
        $msid = 0.00;
        $Query = "SELECT PROFIT_MARGIN FROM principal_supplier_master WHERE Principal_Supplier_Id = $principlaaId";
		$Result = DBConnection::SelectQuery($Query);
		$Row = mysql_fetch_array($Result, MYSQL_ASSOC);		
		$msid = $Row['PROFIT_MARGIN']; 
		return $msid;    
    }
}
