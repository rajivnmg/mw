...
class ItemMaster_Model
{
	...
	public function __construct($itemid,$groupid,$principalid,$itemcodepartno,$itemdescp,$unitid,$itemidentificationmarks,$hsncode,$taxrate,$itemcostprice,$lsc,$usc, $qty, $remarks,$createdId,$createDate,$groupcode,$groupdesc,$principalname,$unitname,$blocked){
	/* EOF for replacing Tarrif with HSN Code by Ayush Giri on 08-06-2017 */
	...
	public static function  LoadItem($itemid,$TAG=null,$ID=null,$page=null,$rp=null,$count=null,$group,$principal,$unit,$identity)
	{
		...
		while ($Row = mysql_fetch_array($result, MYSQL_ASSOC)) {
			...
			$newObj = new ItemMaster_Model($_item_id, $_group_id, $_principal_id, $_item_code_partno, $_item_descp, $_unit_id, $_item_identification_marks, $_hsn_code, $_tax_rate, $_item_cost_price, $_lsc, $_usc, $_qty, $_remarks, $_createdId, $_createDate, $_groupcode, $_group_desc, $_principalname, $_unitname,$_blocked);
			/* BOF for merging Excise and Non Excise quantity  by Ayush Giri on 08-06-2017 */
            $objArray[$i] = $newObj;
            $i++;
		}
	...
	/* Added on 29 June 18 To update the status of item as block/unblock */	
	public static function UpdateItemBlockedStatus($ItemID, $blocked){
	
        $ItemID = $ItemID.trim();    
        $blocked = mysql_escape_string($blocked);        
		$Query = "UPDATE item_master SET blocked = '$blocked' WHERE ItemId = $ItemID";
		$Result = DBConnection::UpdateQuery($Query);
		if($Result == QueryResponse::SUCCESS){
			return QueryResponse::YES;
		}
