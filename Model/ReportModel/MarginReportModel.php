				...
				//$pf_chrg = ($row->pf_chrg / $totalPoVal);
				 ...
				 foreach($row->_itmes as $k) {	
						...
						if (in_array($k->oinvoice_exciseID, $invoice))
						{
							...
							'landing_price'=>number_format($lp, 2, '.', ''),
							'Margin'=>number_format($SallingPrice - ($lp), 2, '.', ''),
							'percentageMargin'=>number_format((($SallingPrice - ($lp))/$SallingPrice)*100, 2, '.', ''),
							'bill_value'=>'---','ms'=>$ms);	
							
						}
						else
						{
							$data[$counter] = array('SN'=>++$i,'oinvoice_No'=>"<a href='".$link."' target='_blank'>$row->oinvoice_No</a>", 'oinvoice_No_excel'=>$row->oinvoice_No,'oinv_date'=>$row->oinv_date, 'Principal_Supplier_Name'=>$row->Principal_Name,'BuyerName'=>$row->Buyer_Name, 'Item_Code_Partno'=>$k->oinv_codePartNo,'codePartNo_desc'=>$k->codePartNo_desc,'issued_qty'=>$k->issued_qty,'Salling'=>number_format($SallingPrice, 2, '.', ''), 'landing_price'=>number_format($lp, 2, '.', ''), 
							'Margin'=>number_format($SallingPrice - ($lp), 2, '.', ''), 
							'percentageMargin'=>number_format((($SallingPrice - ($lp))/$SallingPrice)*100, 2, '.', ''), 
							'bill_value'=>number_format($row->bill_value, 2, '.', ''),'ms'=>$ms);	
					...
					$data[$counter] = array('SN'=>'','oinvoice_No'=>'','oinv_date'=>'', 'oinvoice_No_excel'=>'','Principal_Supplier_Name'=>'','BuyerName'=>'','Item_Code_Partno'=>'','codePartNo_desc'=>'','issued_qty'=>'','Salling'=>'','landing_price'=>'','Margin'=>'','percentageMargin'=>'Grand Total','bill_value'=>$total_val,'ms'=>'');
			} 
			//print_r($data);exit;
 ...
 public static function GetMarginReportNewPDF($todate,$fromdate,$principalid,$buyerid,$txtinvoicenumber,$finyear,$marketsegment)
	...
	public static function GetlandingPrice($principal_invoice)
    {
		...
		if(mysql_num_rows($result) > 0)
		{
			...
			{
				$Query_incoming_inventory = "SELECT IF(ied.s_qty > 0,(ied.taxable_amt/ied.s_qty),(ied.taxable_amt/ied.p_qty)) AS TAXABLE_AMOUNT FROM incominginvoice_excise_detail AS ied JOIN incominginventory iinv ON ied.entryDId = iinv.entryDId WHERE iinv.incoming_inventory_id = ".$principal_invoice.""; 
				//echo $Query; exit;
				...
				if(mysql_num_rows($result_incoming_inventory) > 0)
				{
					...
					{
						return $row_incoming_inventory['TAXABLE_AMOUNT'];
					}
				}
			}
			...
			{
				$Query_incominginvoice_without_excise_old ="SELECT iined.total_landing_price, iined.landing_price FROM incominginvoice_without_excise_detail_old AS iined INNER JOIN incominginvoice_without_excise_old AS iine ON iined.incominginvoice_we = iine.incominginvoice_we WHERE iine.principalID = '".$row['principalId']."' AND iined.itemID_code_partNo = '".$row['code_PartNo']."' AND iined.incominginvoice_we = '".$row['incomingInvNo']."' ORDER BY iined.incominginvoice_we DESC LIMIT 1";
				...
				if(mysql_num_rows($result_incominginvoice_without_excise_old) > 0)
				{
					...
					{
						return $row_incominginvoice_without_excise_old['landing_price']; 				
					}
				}
			}
		}
				 ...
				 foreach($row->_itmes as $k) {	
						...
						$lp = ((self::GetNonExciselandingPrice($row->principalID,$k->_item_id)) * $k->issued_qty);
						 ...
						 if(in_array($row->oinvoice_nexciseID, $invoice)){
							...
							$data[$counter]['Salling'] = number_format($SallingPrice, 2, '.', '');
							$data[$counter]['Margin'] = number_format($SallingPrice - ($lp), 2, '.', '');
							$data[$counter]['percentageMargin'] = number_format((($SallingPrice - ($lp))/$SallingPrice)*100, 2, '.', '');
