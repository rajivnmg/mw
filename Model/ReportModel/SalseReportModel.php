...
class SalseReportModel
{
	...
	public static function getDailySalesReport($todate,$fromdate,$tag,$value,$pid,$bid){	
		...
		/* $Query = "SELECT oe.oinvoice_No, oe.oinvoice_exciseID as id, oe.oinv_date, oe.bill_value, psm.Principal_Supplier_Name, bm.BuyerName, (SELECT SUM(taxable_amt) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID) AS TAXABLE_AMOUNT, (SELECT SUM(cgst_amt) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID) AS CGST_AMOUNT, (SELECT SUM(sgst_amt) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID) AS SGST_AMOUNT, (SELECT SUM(igst_amt) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID) AS IGST_AMOUNT, (SELECT SUM(total) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID) AS TOTAL_AMOUNT  , ((SELECT (SUM(taxable_amt) * (oe.pf_chrg/100)) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID) + oe.p_f_gst_amount) AS P_F_AMOUNT, ((SELECT (SUM(taxable_amt) * (oe.incidental_chrg/100)) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID) + oe.inc_gst_amount) AS INCIDENTAL_AMOUNT, ((SELECT (SUM(taxable_amt) * (oe.insurance_charge/100)) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID) + oe.ins_gst_amount) AS INSURANCE_AMOUNT, ((SELECT (SUM(taxable_amt) * (oe.other_charge/100)) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID) + oe.othc_gst_amount) AS OTHER_AMOUNT, ((IF(((oe.freight_percent IS NULL) OR (oe.freight_percent = '') OR (oe.freight_percent = 0)),oe.freight_amount,(SELECT (SUM(taxable_amt) * (oe.freight_percent/100)) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID))) + oe.freight_gst_amount) AS FREIGHT_AMOUNT FROM outgoinginvoice_excise AS oe INNER JOIN principal_supplier_master AS psm ON psm.Principal_Supplier_Id = oe.principalId INNER JOIN buyer_master as bm on oe.BuyerID = bm.BuyerId WHERE oe.oinv_date BETWEEN '$todate' AND '$fromdate' $opt ORDER BY oe.oinvoice_No, oe.oinv_date ASC"; */
		
		//~ $Query = "SELECT oe.oinvoice_No, oe.oinvoice_exciseID as id, oe.oinv_date, oe.bill_value, psm.Principal_Supplier_Name, bm.BuyerName,oe.freight_gst_amount,oe.freight_amount,oe.freight_gst_rate, (SELECT TRUNCATE(SUM(TRUNCATE(taxable_amt,2)),2) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID) AS TAXABLE_AMOUNT, (SELECT TRUNCATE(SUM(TRUNCATE(cgst_amt,2)),2) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID) AS CGST_AMOUNT, (SELECT TRUNCATE(SUM(TRUNCATE(sgst_amt,2)),2) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID) AS SGST_AMOUNT, (SELECT TRUNCATE(SUM(TRUNCATE(igst_amt,2)),2) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID) AS IGST_AMOUNT, (SELECT TRUNCATE(SUM(TRUNCATE(total,2)),2) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID) AS TOTAL_AMOUNT  , ((SELECT TRUNCATE((TRUNCATE(SUM(TRUNCATE(taxable_amt,2)),2) * TRUNCATE((TRUNCATE(oe.pf_chrg,2)/100),2)),2) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID) + TRUNCATE(oe.p_f_gst_amount,2)) AS P_F_AMOUNT, ((SELECT TRUNCATE((TRUNCATE(SUM(TRUNCATE(taxable_amt,2)),2) * TRUNCATE((TRUNCATE(oe.incidental_chrg,2)/100),2)),2) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID) + TRUNCATE(oe.inc_gst_amount,2)) AS INCIDENTAL_AMOUNT, ((SELECT TRUNCATE((SUM(TRUNCATE(taxable_amt,2)) * TRUNCATE((TRUNCATE(oe.insurance_charge,2)/100),2)),2) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID) + TRUNCATE(oe.ins_gst_amount,2)) AS INSURANCE_AMOUNT, ((SELECT TRUNCATE((SUM(TRUNCATE(taxable_amt,2)) * TRUNCATE((TRUNCATE(oe.other_charge,2)/100),2)),2) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID) + TRUNCATE(oe.othc_gst_amount,2)) AS OTHER_AMOUNT, ((IF(((oe.freight_percent IS NULL) OR (oe.freight_percent = '') OR (oe.freight_percent = 0)),TRUNCATE(oe.freight_amount,2),(SELECT TRUNCATE((SUM(TRUNCATE(taxable_amt,2)) * TRUNCATE((TRUNCATE(oe.freight_percent,2)/100),2)),2) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID))) + TRUNCATE(oe.freight_gst_amount,2)) AS FREIGHT_AMOUNT FROM outgoinginvoice_excise AS oe INNER JOIN principal_supplier_master AS psm ON psm.Principal_Supplier_Id = oe.principalId INNER JOIN buyer_master as bm on oe.BuyerID = bm.BuyerId WHERE oe.oinv_date BETWEEN '$todate' AND '$fromdate' $opt ORDER BY oe.oinvoice_No, oe.oinv_date ASC";
		
			//~ $Query = "SELECT oe.oinvoice_No, oe.oinvoice_exciseID as id, oe.oinv_date, oe.bill_value, psm.Principal_Supplier_Name, bm.BuyerName,oe.freight_gst_amount,oe.freight_amount,oe.freight_gst_rate,oe.pf_chrg,oe.p_f_gst_rate,oe.p_f_gst_amount,oe.incidental_chrg,oe.inc_gst_rate,oe.inc_gst_amount,oe.insurance_charge,oe.ins_gst_rate,oe.ins_gst_amount,oe.other_charge,oe.othc_gst_rate,oe.othc_gst_amount, (SELECT TRUNCATE(SUM(TRUNCATE(taxable_amt,2)),2) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID) AS TAXABLE_AMOUNT, (SELECT TRUNCATE(SUM(TRUNCATE(cgst_amt,2)),2) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID) AS CGST_AMOUNT, (SELECT TRUNCATE(SUM(TRUNCATE(sgst_amt,2)),2) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID) AS SGST_AMOUNT, (SELECT TRUNCATE(SUM(TRUNCATE(igst_amt,2)),2) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID) AS IGST_AMOUNT, (SELECT TRUNCATE(SUM(TRUNCATE(total,2)),2) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID) AS TOTAL_AMOUNT  , ((SELECT TRUNCATE((TRUNCATE(SUM(TRUNCATE(taxable_amt,2)),2) * TRUNCATE((TRUNCATE(oe.pf_chrg,2)/100),2)),2) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID) + TRUNCATE(oe.p_f_gst_amount,2)) AS P_F_AMOUNT, ((SELECT TRUNCATE((TRUNCATE(SUM(TRUNCATE(taxable_amt,2)),2) * TRUNCATE((TRUNCATE(oe.incidental_chrg,2)/100),2)),2) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID) + TRUNCATE(oe.inc_gst_amount,2)) AS INCIDENTAL_AMOUNT, ((SELECT TRUNCATE((SUM(TRUNCATE(taxable_amt,2)) * TRUNCATE((TRUNCATE(oe.insurance_charge,2)/100),2)),2) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID) + TRUNCATE(oe.ins_gst_amount,2)) AS INSURANCE_AMOUNT, ((SELECT TRUNCATE((SUM(TRUNCATE(taxable_amt,2)) * TRUNCATE((TRUNCATE(oe.other_charge,2)/100),2)),2) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID) + TRUNCATE(oe.othc_gst_amount,2)) AS OTHER_AMOUNT, ((IF(((oe.freight_percent IS NULL) OR (oe.freight_percent = '') OR (oe.freight_percent = 0)),TRUNCATE(oe.freight_amount,2),(SELECT TRUNCATE((SUM(TRUNCATE(taxable_amt,2)) * TRUNCATE((TRUNCATE(oe.freight_percent,2)/100),2)),2) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID))) + TRUNCATE(oe.freight_gst_amount,2)) AS FREIGHT_AMOUNT FROM outgoinginvoice_excise AS oe INNER JOIN principal_supplier_master AS psm ON psm.Principal_Supplier_Id = oe.principalId INNER JOIN buyer_master as bm on oe.BuyerID = bm.BuyerId WHERE oe.oinv_date BETWEEN '$todate' AND '$fromdate' $opt ORDER BY oe.oinvoice_No, oe.oinv_date ASC";
			$Query = "SELECT oe.oinvoice_No, oe.oinvoice_exciseID as id, oe.oinv_date, oe.bill_value, psm.Principal_Supplier_Name, bm.BuyerName,oe.freight_gst_amount,oe.freight_amount,oe.freight_gst_rate,oe.pf_chrg,oe.p_f_gst_rate,oe.p_f_gst_amount,oe.incidental_chrg,oe.inc_gst_rate,oe.inc_gst_amount,oe.insurance_charge,oe.ins_gst_rate,oe.ins_gst_amount,oe.other_charge,oe.othc_gst_rate,oe.othc_gst_amount, (SELECT TRUNCATE(SUM(TRUNCATE(taxable_amt,2)),2) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID) AS TAXABLE_AMOUNT, (SELECT TRUNCATE(SUM(TRUNCATE(cgst_amt,2)),2) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID) AS CGST_AMOUNT, (SELECT TRUNCATE(SUM(TRUNCATE(sgst_amt,2)),2) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID) AS SGST_AMOUNT, (SELECT TRUNCATE(SUM(TRUNCATE(igst_amt,2)),2) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID) AS IGST_AMOUNT, (SELECT TRUNCATE(SUM(TRUNCATE(total,2)),2) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID) AS TOTAL_AMOUNT  , ((SELECT TRUNCATE((TRUNCATE(SUM(TRUNCATE(taxable_amt,2)),2) * TRUNCATE((TRUNCATE(oe.pf_chrg,2)/100),2)),2) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID)) AS P_F_AMOUNT, ((SELECT TRUNCATE((TRUNCATE(SUM(TRUNCATE(taxable_amt,2)),2) * TRUNCATE((TRUNCATE(oe.incidental_chrg,2)/100),2)),2) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID)) AS INCIDENTAL_AMOUNT, ((SELECT TRUNCATE((SUM(TRUNCATE(taxable_amt,2)) * TRUNCATE((TRUNCATE(oe.insurance_charge,2)/100),2)),2) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID)) AS INSURANCE_AMOUNT, ((SELECT TRUNCATE((SUM(TRUNCATE(taxable_amt,2)) * TRUNCATE((TRUNCATE(oe.other_charge,2)/100),2)),2) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID)) AS OTHER_AMOUNT, ((IF(((oe.freight_percent IS NULL) OR (oe.freight_percent = '') OR (oe.freight_percent = 0)),TRUNCATE(oe.freight_amount,2),(SELECT TRUNCATE((SUM(TRUNCATE(taxable_amt,2)) * TRUNCATE((TRUNCATE(oe.freight_percent,2)/100),2)),2) FROM outgoinginvoice_excise_detail oied WHERE oied.oinvoice_exciseID = oe.oinvoice_exciseID))) + TRUNCATE(oe.freight_gst_amount,2)) AS FREIGHT_AMOUNT FROM outgoinginvoice_excise AS oe INNER JOIN principal_supplier_master AS psm ON psm.Principal_Supplier_Id = oe.principalId INNER JOIN buyer_master as bm on oe.BuyerID = bm.BuyerId WHERE oe.oinv_date BETWEEN '$todate' AND '$fromdate' $opt ORDER BY oe.oinvoice_No, oe.oinv_date ASC";
        ...
        if(mysql_num_rows($result) > 0){		
			 while($row = mysql_fetch_array($result,MYSQL_ASSOC)){
				...
				//~ if($row['IGST_AMOUNT'] > 0){					
					//~ $igst_famt = ($row['P_F_AMOUNT']+$row['INCIDENTAL_AMOUNT']+$row['INSURANCE_AMOUNT']+$row['freight_amount']);
					
				//~ }else if($row['SGST_AMOUNT'] > 0 && $row['CGST_AMOUNT'] > 0 ){
					//~ $cgst_famt = ($row['P_F_AMOUNT']+$row['INCIDENTAL_AMOUNT']+$row['INSURANCE_AMOUNT']+$row['freight_amount'])/2;
					//~ $sgst_famt = ($row['P_F_AMOUNT']+$row['INCIDENTAL_AMOUNT']+$row['INSURANCE_AMOUNT']+$row['freight_amount'])/2;
										
				//~ }else if($row['SGST_AMOUNT'] > 0){
					
					//~ $sgst_famt = $row['P_F_AMOUNT']+$row['INCIDENTAL_AMOUNT']+$row['INSURANCE_AMOUNT']+$row['freight_amount'];
				//~ }else{
					//~ $cgst_famt = $row['P_F_AMOUNT']+$row['INCIDENTAL_AMOUNT']+$row['INSURANCE_AMOUNT']+$row['freight_amount'];
				//~ }
				...
				$gst_rate = 0;
				if($p_f_gst_rate > 0){
					$gst_rate = $p_f_gst_rate;
				}else if($ins_gst_rate > 0){
					$gst_rate = $ins_gst_rate;
				}else if($inc_gst_rate > 0){
					$gst_rate = $inc_gst_rate;
				
				}else if($othc_gst_rate > 0){
					$gst_rate = $othc_gst_rate;
				}
			
				
				if($row['IGST_AMOUNT'] > 0){	
									
					$igst_famt = ($row['p_f_gst_amount']+$row['inc_gst_amount']+$row['ins_gst_amount']+$row['freight_amount']+$row['othc_gst_amount']);					
					$igst_famt = (($igst_famt * $gst_rate)/100);
				}else if($row['SGST_AMOUNT'] > 0 && $row['CGST_AMOUNT'] > 0 ){
					$cgst_famt = ($row['p_f_gst_amount']+$row['inc_gst_amount']+$row['ins_gst_amount']+$row['freight_amount']+$row['othc_gst_amount'])/2;
					$sgst_famt = ($row['p_f_gst_amount']+$row['inc_gst_amount']+$row['ins_gst_amount']+$row['freight_amount']+$row['othc_gst_amount'])/2;
					$cgst_famt = (($cgst_famt * $gst_rate)/100);
					$sgst_famt = (($sgst_famt * $gst_rate)/100);
										
				}else if($row['SGST_AMOUNT'] > 0){
					
					$sgst_famt = ($row['p_f_gst_amount']+$row['inc_gst_amount']+$row['ins_gst_amount']+$row['freight_amount']+$row['othc_gst_amount']);
					
					$sgst_famt = (($sgst_famt * $gst_rate)/100);
				}else{
					$cgst_famt = ($row['p_f_gst_amount']+$row['inc_gst_amount']+$row['ins_gst_amount']+$row['freight_amount']+$row['othc_gst_amount']);
					$cgst_famt = (($cgst_famt * $gst_rate)/100);
				}
				$data[$counter] = array(
										'SN'=>++$i,
										...
										'taxable_amt'=>number_format($row['TAXABLE_AMOUNT'], 2, '.',''),
										'cgst_amt'=>number_format($row['CGST_AMOUNT'], 2, '.',''),
										'sgst_amt'=>number_format($row['SGST_AMOUNT'], 2, '.',''),
										'igst_amt'=>number_format($row['IGST_AMOUNT'], 2, '.',''),
										'p_f_amt'=>number_format($row['P_F_AMOUNT'], 2, '.',''),
										'inc_amt'=>number_format($row['INCIDENTAL_AMOUNT'], 2, '.',''),
										'ins_amt'=>number_format($row['INSURANCE_AMOUNT'], 2, '.',''),
										'other_amt'=>number_format($row['OTHER_AMOUNT'], 2, '.',''),
										'cgst_camt'=>number_format($cgst_famt, 2, '.',''),
										'sgst_camt'=>number_format($sgst_famt, 2, '.',''),
										'igst_camt'=>number_format($igst_famt, 2, '.',''),
										//'freight_amount'=>number_format($row['FREIGHT_AMOUNT'], 2, '.',''),
										'freight_amount'=>number_format($row['freight_amount'], 2, '.',''),
										'bill_value'=>number_format($row['bill_value'], 2, '.','')
										);					
					$total_val = $total_val + $row['bill_value'];		 
	...
	public static function GetPurchaseReportNew($todate,$fromdate,$marketseg,$invoicenumber,$pid,$itemid){		
		...
		$Query1="(SELECT ie.principal_inv_no as invoice,ie.principal_inv_date as invoiceDate,ie.total_bill_val as totalbill,IF(ied.s_qty > 0, ied.s_qty, ied.p_qty) as qty,ied.basic_purchase_price as unitrate,psm.Principal_Supplier_Name,im.Item_Code_Partno,im.Item_Desc,ie.msid, ied.cgst_amt AS CGST_AMOUNT, ied.sgst_amt AS SGST_AMOUNT, ied.igst_amt AS IGST_AMOUNT, ied.packing_amt AS PACKING_AMOUNT, ied.insurance_amt AS INSURANCE_AMOUNT, ied.freight_amt AS FREIGHT_AMOUNT, ied.other_amt AS OTHER_AMOUNT, (ied.total - ied.discounted_amt) AS DISCOUNT FROM incominginvoice_excise AS ie 
		INNER JOIN incominginvoice_excise_detail AS ied ON ie.entryId = ied.entryId
		INNER JOIN item_master AS im ON ied.itemID_code_partNo = im.ItemId
		INNER JOIN principal_supplier_master AS psm ON psm.Principal_Supplier_Id = ie.principalId
		WHERE ie.principal_inv_date BETWEEN '$todate' AND '$fromdate' $opt1 ) ";		
		
		$Query2="(SELECT iwe.incoming_inv_no_p as invoice,iwe.principal_inv_date as invoiceDate,iwe.tot_bill_val as totalbill, iwed.qty as qty, iwed.rate as unitrate,  psm.Principal_Supplier_Name, im.Item_Code_Partno, im.Item_Desc,iwe.msid, '0' AS CGST_AMOUNT, '0' AS SGST_AMOUNT, '0' AS IGST_AMOUNT, '0' AS PACKING_AMOUNT, '0' AS INSURANCE_AMOUNT, '0' AS FREIGHT_AMOUNT, '0' AS OTHER_AMOUNT, '0' AS DISCOUNT
		FROM incominginvoice_without_excise AS iwe
		INNER JOIN incominginvoice_without_excise_detail AS iwed ON iwed.incominginvoice_we = iwe.incominginvoice_we
		INNER JOIN item_master AS im ON iwed.itemID_code_partNo = im.ItemId
		INNER JOIN principal_supplier_master AS psm ON psm.Principal_Supplier_Id = iwe.principalID
        ...
        if(mysql_num_rows($result) > 0){		
			...
			if (in_array($row['invoice'], $invoice)){
				//$data[$counter] = array('SN'=>++$i,'invoice_No'=>$row['invoice'],'inv_date'=>$row['invoiceDate'],'Principal_Supplier_Name'=>$row['Principal_Supplier_Name'],'Item_Code_Partno'=>$row['Item_Code_Partno'],'Item_Desc'=>$row['Item_Desc'],'qty'=>$row['qty'],'unitrate'=>$row['unitrate'],'basic_value'=>($row['qty']*$row['unitrate']),'bill_value'=>'---');	
				$data[$counter] = array('SN'=>--$i, 'invoice_No'=>$row['invoice'], 'inv_date'=>$row['invoiceDate'], 'Principal_Supplier_Name'=>$row['Principal_Supplier_Name'], 'Item_Code_Partno'=>$row['Item_Code_Partno'], 'Item_Desc'=>$row['Item_Desc'], 'qty'=>$row['qty'], 'unitrate'=>$row['unitrate'], 'basic_value'=>(($row['qty']*$row['unitrate'])- $row['DISCOUNT']), 'bill_value'=>'---', 'CGST_AMOUNT'=>$row['CGST_AMOUNT'], 'SGST_AMOUNT'=>$row['SGST_AMOUNT'], 'IGST_AMOUNT'=>$row['IGST_AMOUNT'], 'PACKING_AMOUNT'=>$row['PACKING_AMOUNT'], 'INSURANCE_AMOUNT'=>$row['INSURANCE_AMOUNT'], 'FREIGHT_AMOUNT'=>$row['FREIGHT_AMOUNT'], 'OTHER_AMOUNT'=>$row['OTHER_AMOUNT'], 'DISCOUNT'=>$row['DISCOUNT'], 'total_value'=>(($row['qty']*$row['unitrate'])- $row['DISCOUNT'] + $row['CGST_AMOUNT'] + $row['SGST_AMOUNT'] + $row['IGST_AMOUNT'] + $row['PACKING_AMOUNT'] + $row['INSURANCE_AMOUNT'] + $row['FREIGHT_AMOUNT'] + $row['OTHER_AMOUNT']));	
			  }else{
				//$data[$counter] = array('SN'=>++$i,'invoice_No'=>$row['invoice'],'inv_date'=>$row['invoiceDate'],'Principal_Supplier_Name'=>$row['Principal_Supplier_Name'],'Item_Code_Partno'=>$row['Item_Code_Partno'],'Item_Desc'=>$row['Item_Desc'],'qty'=>$row['qty'],'unitrate'=>$row['unitrate'],'basic_value'=>($row['qty']*$row['unitrate']),'bill_value'=>$row['totalbill']);	
				$data[$counter] = array('SN'=>--$i,'invoice_No'=>$row['invoice'],'inv_date'=>$row['invoiceDate'],'Principal_Supplier_Name'=>$row['Principal_Supplier_Name'],'Item_Code_Partno'=>$row['Item_Code_Partno'],'Item_Desc'=>$row['Item_Desc'],'qty'=>$row['qty'],'unitrate'=>$row['unitrate'],'basic_value'=>(($row['qty']*$row['unitrate'])- $row['DISCOUNT']),'bill_value'=>$row['totalbill'], 'CGST_AMOUNT'=>$row['CGST_AMOUNT'], 'SGST_AMOUNT'=>$row['SGST_AMOUNT'], 'IGST_AMOUNT'=>$row['IGST_AMOUNT'], 'PACKING_AMOUNT'=>$row['PACKING_AMOUNT'], 'INSURANCE_AMOUNT'=>$row['INSURANCE_AMOUNT'], 'FREIGHT_AMOUNT'=>$row['FREIGHT_AMOUNT'], 'OTHER_AMOUNT'=>$row['OTHER_AMOUNT'], 'DISCOUNT'=>$row['DISCOUNT'], 'total_value'=>(($row['qty']*$row['unitrate'])- $row['DISCOUNT'] + $row['CGST_AMOUNT'] + $row['SGST_AMOUNT'] + $row['IGST_AMOUNT'] + $row['PACKING_AMOUNT'] + $row['INSURANCE_AMOUNT'] + $row['FREIGHT_AMOUNT'] + $row['OTHER_AMOUNT']));	
				$total_val = $total_val + $row['totalbill'];
				$invoice[] = $row['invoice'];
			  }
			...
			//$data[$counter] = array('SN'=>'','invoice_No'=>'','inv_date'=>'','Principal_Supplier_Name'=>'','Item_Code_Partno'=>'','Item_Desc'=>'','qty'=>'','unitrate'=>'','basic_value'=>'Total : ','bill_value'=>$total_val);
			$data_1[$counter] = array('SN'=>'','invoice_No'=>'','inv_date'=>'','Principal_Supplier_Name'=>'','Item_Code_Partno'=>'','Item_Desc'=>'','qty'=>'','unitrate'=>'','basic_value'=>'','bill_value'=>$total_val, 'CGST_AMOUNT'=>'', 'SGST_AMOUNT'=>'', 'IGST_AMOUNT'=>'', 'PACKING_AMOUNT'=>'', 'INSURANCE_AMOUNT'=>'', 'FREIGHT_AMOUNT'=>'', 'OTHER_AMOUNT'=>'', 'DISCOUNT'=>'', 'total_value'=>'Total : ');
		}   
		return $data_1;
    }
