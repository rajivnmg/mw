...
//Function added on August 4 ,2018 for stock level report
 public static function GetExciseStockLevelReport()
    {        
 $Query = "select (@cnt := @cnt + 1) AS SN,im.Item_Code_Partno, im.Item_Desc, im.Tarrif_Heading,
inv.tot_Qty,um.UNITNAME,im.Cost_Price,im.Lsc,im.Usc
,inv.tot_Qty * im.Cost_Price as total_price, gm.GroupId, gm.GroupDesc ,im.ItemId
from item_master as im
inner join unit_master as um ON im.UnitId = um.UnitId
inner join inventory as inv on im.ItemId = inv.code_partNo
...
//Function added on August 4 ,2018 for stock level report
 public static function GetExciseStockLevelReportEMAIL()
    {        
 $Query = "select (@cnt := @cnt + 1) AS SN,im.Item_Code_Partno, im.Item_Desc, im.Tarrif_Heading,
inv.tot_Qty,um.UNITNAME,im.Cost_Price,im.Lsc,im.Usc
,inv.tot_Qty * im.Cost_Price as total_price, gm.GroupId, gm.GroupDesc ,im.ItemId
...
left join group_master as gm on im.GroupId = gm.GroupId
CROSS JOIN (SELECT @cnt := 0) AS dummy
where inv.tot_Qty  > 0 AND im.Usc != 0 AND im.Lsc != 0 order by SN ASC;";
//echo '<br/>Query --> '.$Query;
        $result = DBConnection::SelectQuery($Query);
        return $result;
    }
...
where oie.oinv_date BETWEEN '".$todate."' AND '".$fromdate."' $cond AND ($finyrs) order by SN ASC";
        ...
        */
          ...
          /* $Query = "select (@cnt := @cnt + 1) AS SN, oie.oinvoice_No, oie.oinv_date, bm.BuyerCode,
bm.BuyerName ,im.Item_Code_Partno, im.Item_Desc,oied.issued_qty ,um.UNITNAME ,oied.oinv_price ,oied.taxable_amt,
((oied.oinv_price * oied.issued_qty) - oied.taxable_amt) AS DISCOUNT, oied.cgst_amt, oied.sgst_amt, oied.igst_amt, oied.total, po.executiveId,oie.msid,oie.pono,oied.codePartNo_desc,oie.principalID, psm.Principal_Supplier_Name, oie.bill_value
...
where oie.oinv_date BETWEEN '".$todate."' AND '".$fromdate."' $cond AND ($finyrs) order by oie.oinvoice_No ASC"; */
		...
		$Query = "select (@cnt := @cnt + 1) AS SN, oie.oinvoice_No, oie.oinv_date, bm.BuyerCode,bm.BuyerName, im.Item_Code_Partno, im.Item_Desc,oied.issued_qty ,um.UNITNAME ,oied.oinv_price ,oied.taxable_amt, ((oied.oinv_price * oied.issued_qty) - oied.taxable_amt) AS DISCOUNT, oied.cgst_amt, oied.sgst_amt, oied.igst_amt, oied.total, po.executiveId, oie.msid,oie.pono, oied.codePartNo_desc, oie.principalID, psm.Principal_Supplier_Name, oie.bill_value, IFNULL(((SELECT TRUNCATE((TRUNCATE(SUM(TRUNCATE(taxable_amt,2)),2) * TRUNCATE((TRUNCATE(oie.pf_chrg,2)/100),2)),2) FROM outgoinginvoice_excise_detail oed WHERE oed.oinvoice_exciseID = oie.oinvoice_exciseID) + TRUNCATE(oie.p_f_gst_amount,2)),0) AS P_F_AMOUNT, IFNULL(((SELECT TRUNCATE((TRUNCATE(SUM(TRUNCATE(taxable_amt,2)),2) * TRUNCATE((TRUNCATE(oie.incidental_chrg,2)/100),2)),2) FROM outgoinginvoice_excise_detail oed WHERE oed.oinvoice_exciseID = oie.oinvoice_exciseID) + TRUNCATE(oie.inc_gst_amount,2)),0) AS INCIDENTAL_AMOUNT, IFNULL(((SELECT TRUNCATE((SUM(TRUNCATE(taxable_amt,2)) * TRUNCATE((TRUNCATE(oie.insurance_charge,2)/100),2)),2) FROM outgoinginvoice_excise_detail oed WHERE oed.oinvoice_exciseID = oie.oinvoice_exciseID) + TRUNCATE(oie.ins_gst_amount,2)),0) AS INSURANCE_AMOUNT, IFNULL(((SELECT TRUNCATE((SUM(TRUNCATE(taxable_amt,2)) * TRUNCATE((TRUNCATE(oie.other_charge,2)/100),2)),2) FROM outgoinginvoice_excise_detail oed WHERE oed.oinvoice_exciseID = oie.oinvoice_exciseID) + TRUNCATE(oie.othc_gst_amount,2)),0) AS OTHER_AMOUNT, IFNULL(((IF(((oie.freight_percent IS NULL) OR (oie.freight_percent = '') OR (oie.freight_percent = 0)),TRUNCATE(oie.freight_amount,2),(SELECT TRUNCATE((SUM(TRUNCATE(taxable_amt,2)) * TRUNCATE((TRUNCATE(oie.freight_percent,2)/100),2)),2) FROM outgoinginvoice_excise_detail oed WHERE oed.oinvoice_exciseID = oie.oinvoice_exciseID))) + TRUNCATE(oie.freight_gst_amount,2)),0) AS FREIGHT_AMOUNT from outgoinginvoice_excise_detail as oied inner join outgoinginvoice_excise as oie on oied.oinvoice_exciseID = oie.oinvoice_exciseID inner join outgoinginvoice_excise_mapping as oim ON oie.oinvoice_exciseID=oim.inner_outgoingInvoiceEx inner join buyer_master as bm ON oie.BuyerID = bm.BuyerId inner join item_master as im on im.ItemId = oied.codePartNo_desc inner join unit_master as um ON im.UnitId = um.UnitId INNER join purchaseorder as po ON oie.pono = po.bpoId inner join purchaseorder_detail as pod ON pod.bpoId = po.bpoId AND oied.oinv_price = pod.po_price INNER JOIN principal_supplier_master psm ON psm.Principal_Supplier_Id = oie.principalID CROSS JOIN (SELECT @cnt := 0) AS dummy where oie.oinv_date BETWEEN '".$todate."' AND '".$fromdate."' $cond AND ($finyrs) order by oie.oinvoice_No DESC";
	...
	if(mysql_num_rows($result) > 0)
	{
            while($row = mysql_fetch_assoc($result))
            {
				...
				{
					$data[$counter] = array('SN'=>--$i,'oinvoice_No'=>$row['oinvoice_No'], 'oinv_date'=>$row['oinv_date'],'BuyerCode'=>$row['BuyerCode'], 'BuyerName'=>$row['BuyerName'],'Item_Code_Partno'=>$row['Item_Code_Partno'], 'Item_Desc'=>$row['Item_Desc'], 'issued_qty'=>$row['issued_qty'], 'UNITNAME'=>$row['UNITNAME'], 'po_price'=>$row['oinv_price'],'salesExecutive'=>$row['executiveId'],'totalprice'=>$row['total'],'marketsegment'=>$marketsegment,'PRINCIPAL_NAME'=>$row['Principal_Supplier_Name'], 'DISCOUNT'=>number_format($row['DISCOUNT'], 2, '.', ''), 'CGST_AMOUNT'=>number_format($row['cgst_amt'], 2, '.', ''), 'SGST_AMOUNT'=>number_format($row['sgst_amt'], 2, '.', ''), 'IGST_AMOUNT'=>number_format($row['igst_amt'], 2, '.', ''), 'TAXABLE_AMOUNT'=>number_format($row['taxable_amt'], 2, '.', ''), 'INVOICE_VALUE'=>'---', 'P_F_AMOUNT'=>'---', 'INCIDENTAL_AMOUNT'=>'---', 'INSURANCE_AMOUNT'=>'---', 'OTHER_AMOUNT'=>'---', 'FREIGHT_AMOUNT'=>'---');
				}
				else
				{
					$data[$counter] = array('SN'=>--$i,'oinvoice_No'=>$row['oinvoice_No'], 'oinv_date'=>$row['oinv_date'],'BuyerCode'=>$row['BuyerCode'], 'BuyerName'=>$row['BuyerName'],'Item_Code_Partno'=>$row['Item_Code_Partno'], 'Item_Desc'=>$row['Item_Desc'], 'issued_qty'=>$row['issued_qty'], 'UNITNAME'=>$row['UNITNAME'], 'po_price'=>$row['oinv_price'],'salesExecutive'=>$row['executiveId'],'totalprice'=>$row['total'],'marketsegment'=>$marketsegment,'PRINCIPAL_NAME'=>$row['Principal_Supplier_Name'], 'DISCOUNT'=>number_format($row['DISCOUNT'], 2, '.', ''), 'CGST_AMOUNT'=>number_format($row['cgst_amt'], 2, '.', ''), 'SGST_AMOUNT'=>number_format($row['sgst_amt'], 2, '.', ''), 'IGST_AMOUNT'=>number_format($row['igst_amt'], 2, '.', ''), 'TAXABLE_AMOUNT'=>number_format($row['taxable_amt'], 2, '.', ''), 'INVOICE_VALUE'=>number_format($row['bill_value'], 2, '.', ''), 'P_F_AMOUNT'=>number_format($row['P_F_AMOUNT'], 2, '.', ''), 'INCIDENTAL_AMOUNT'=>number_format($row['INCIDENTAL_AMOUNT'], 2, '.', ''), 'INSURANCE_AMOUNT'=>number_format($row['INSURANCE_AMOUNT'], 2, '.', ''), 'OTHER_AMOUNT'=>number_format($row['OTHER_AMOUNT'], 2, '.', ''), 'FREIGHT_AMOUNT'=>number_format($row['FREIGHT_AMOUNT'], 2, '.', ''));
					$total_invoice = $total_invoice + $row['bill_value'];
            ...
            $data_1[$counter] = array('SN'=>'','oinvoice_No'=>'','oinv_date'=>'',
                    'BuyerCode'=>'','BuyerName'=>'','Item_Code_Partno'=>'', 'Item_Desc'=>'',
                    'issued_qty'=>'','UNITNAME'=>'','po_price'=>'','salesExecutive'=>'','totalprice'=>$total,'marketsegment'=>'','PRINCIPAL_NAME'=>'', 'DISCOUNT'=>'', 'CGST_AMOUNT'=>'', 'SGST_AMOUNT'=>'', 'IGST_AMOUNT'=>'Gross Total', 'TAXABLE_AMOUNT'=>'', 'INVOICE_VALUE'=>number_format($total_invoice, 2, '.', ''), 'P_F_AMOUNT'=>'', 'INCIDENTAL_AMOUNT'=>'', 'INSURANCE_AMOUNT'=>'', 'OTHER_AMOUNT'=>'', 'FREIGHT_AMOUNT'=>'');
            $counter++;
...
where inv.tot_nonExciseQty  > 0 AND im.GroupId = $Groupid order by SN ASC;";
	...
	public static function GetlandingPrice($principal_invoice)
    {
		...
		$Query = "SELECT IF(principal_inv_no IS NULL, 'NEW', incomingInvTyp) AS INVOICE_TYPE, entryDId, incomingInvNo, principalId, code_PartNo FROM incominginventory WHERE incoming_inventory_id = '".$principal_invoice."' ";
		...
		if(mysql_num_rows($result) > 0)
		{
			...
			{
				$Query_incoming_inventory = "SELECT IF(ied.s_qty > 0,(ied.discounted_amt/ied.s_qty),(ied.discounted_amt/ied.p_qty)) AS DISCOUNTED_AMOUNT FROM incominginvoice_excise_detail AS ied JOIN incominginventory iinv ON ied.entryDId = iinv.entryDId WHERE iinv.incoming_inventory_id = ".$principal_invoice.""; 
				//echo $Query; exit;
				$result_incoming_inventory = DBConnection::SelectQuery($Query_incoming_inventory); 
				
				if(mysql_num_rows($result_incoming_inventory) > 0)
				{
					while($row_incoming_inventory = mysql_fetch_array($result_incoming_inventory,MYSQL_ASSOC))
					{
						return $row_incoming_inventory['DISCOUNTED_AMOUNT'];
					}
				}
			}
			else if($row['INVOICE_TYPE'] == 'E')
			{
				$Query_incominginvoice_excise_detail_old ="SELECT ied.basic_purchase_price FROM incominginvoice_excise_detail_old AS ied WHERE ied.entryDId = '".$row['entryDId']."'";
				
				$result_incominginvoice_excise_detail_old = DBConnection::SelectQuery($Query_incominginvoice_excise_detail_old);
				if(mysql_num_rows($result_incominginvoice_excise_detail_old) > 0)
				{
					while($row_incominginvoice_excise_detail_old = mysql_fetch_array($result_incominginvoice_excise_detail_old,MYSQL_ASSOC))
					{
						return $row_incominginvoice_excise_detail_old['basic_purchase_price']; 				
					}
				}
			}
			else
			{
				$Query_incominginvoice_without_excise_old ="SELECT iined.rate FROM incominginvoice_without_excise_detail_old AS iined INNER JOIN incominginvoice_without_excise_old AS iine ON iined.incominginvoice_we = iine.incominginvoice_we WHERE iine.principalID = '".$row['principalId']."' AND iined.itemID_code_partNo = '".$row['code_PartNo']."' AND iined.incominginvoice_we = '".$row['incomingInvNo']."' ORDER BY iined.incominginvoice_we DESC LIMIT 1";
				
				$result_incominginvoice_without_excise_old = DBConnection::SelectQuery($Query_incominginvoice_without_excise_old);
				if(mysql_num_rows($result_incominginvoice_without_excise_old) > 0)
				{
					while($row_incominginvoice_without_excise_old = mysql_fetch_array($result_incominginvoice_without_excise_old,MYSQL_ASSOC))
					{
						return $row_incominginvoice_without_excise_old['rate']; 				
					}
				}
			}
		}
...
$Query = "select (@cnt := @cnt + 1) AS SN, oie.oinvoice_No, oie.oinv_date, bm.BuyerCode,
bm.BuyerName ,im.Item_Code_Partno, im.Item_Desc,oied.issued_qty ,um.UNITNAME , oied.iinv_no ,oie.msid,oim.finyear,oie.pono,oied.codePartNo_desc,oie.principalID, psm.Principal_Supplier_Name from outgoinginvoice_excise_detail as oied
LEFT join outgoinginvoice_excise as oie on oied.oinvoice_exciseID = oie.oinvoice_exciseID
LEFT join outgoinginvoice_excise_mapping as oim ON oie.oinvoice_exciseID=oim.inner_outgoingInvoiceEx
LEFT join buyer_master as bm ON oie.BuyerID = bm.BuyerId
LEFT join item_master as im on im.ItemId = oied.codePartNo_desc
LEFT join unit_master as um ON im.UnitId = um.UnitId
LEFT join incominginventory as iinv ON oied.iinv_no = iinv.incoming_inventory_id
LEFT join incominginvoice_excise_detail as iod ON iinv.entryDId = iod.entryDId
LEFT JOIN principal_supplier_master psm ON psm.Principal_Supplier_Id = oie.principalID
CROSS JOIN (SELECT @cnt := 0) AS dummy
where oie.oinv_date BETWEEN '".$todate."' AND '".$fromdate."' $cond AND ($finyrs) order by oie.oinvoice_No ASC";
	...
	if(mysql_num_rows($result) > 0)
	{
            while($row = mysql_fetch_assoc($result))
            {
				...
				$basic_price = self::GetlandingPrice($row['iinv_no']);
                ...
                $data[$counter] = array('SN'=>++$i,'oinvoice_No'=>$row['oinvoice_No'],'oinv_date'=>$row['oinv_date'], 'BuyerCode'=>$row['BuyerCode'],'BuyerName'=>$row['BuyerName'],'Item_Code_Partno'=>$row['Item_Code_Partno'], 'Item_Desc'=>$row['Item_Desc'],
                    'issued_qty'=>$row['issued_qty'],'UNITNAME'=>$row['UNITNAME'],'po_price'=>number_format($basic_price, 2, '.', ''),'totalprice'=>number_format(($basic_price * $row['issued_qty']), 2, '.', ''),'marketsegment'=>$marketsegment, 'PRINCIPAL_NAME'=>$row['Principal_Supplier_Name']);
                $counter++;
                $total = $total + ($basic_price * $row['issued_qty']);
            }
            
             $data[$counter] = array('SN'=>$row[0],'oinvoice_No'=>'','oinv_date'=>'',
                    'BuyerCode'=>'','BuyerName'=>'','Item_Code_Partno'=>'', 'Item_Desc'=>'',
                    'issued_qty'=>'','UNITNAME'=>'','po_price'=>'Gross Total','totalprice'=>number_format($total, 2, '.', ''),'marketsegment'=>'', 'PRINCIPAL_NAME'=>'');
              $counter++;
	}
        return $data;
    }
...
where oine.oinv_date BETWEEN '".$todate."' AND '".$fromdate."' $cond AND ($finyrs) order by oine.oinvoice_No ASC";
    ...
    // Function add to get the excise none-excise quentiry of the basis of date for report to check the all item stock on the basis of date ON 12-May-2016 from Transaction Table
	 public static function  checkInventoryByDateFromTransaction($Type,$code_partNo,$curentFinYear,$date){
		...
		/* BOF to correct Stock Statement By Date Report by Ayush Giri on 19-12-2017 */ 
		 ...
		 $Query = "select IFNULL(tds.balance_qty,0) as balance_qty,tds.code_partNo,td.transactionDate,tds.excise_nonexcise_TAG FROM transaction_detail AS tds
		 inner join transaction AS td ON td.transactionId = tds.transactionId 
		 WHERE tds.code_partNo ='$code_partNo' AND  td.transactionDate <= '$date' order by tds.transactiondId DESC LIMIT 1";
		/* EOF to correct Stock Statement By Date Report by Ayush Giri on 19-12-2017 */
