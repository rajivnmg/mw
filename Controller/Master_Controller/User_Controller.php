...
//function to update the password of the user from the admin added on July 7, 2017
else if($page=="UpdateUserPassword")
{
    ...
    {
     case QueryModel::UPDATE:
         $Data = $_REQUEST['UpdatePassword'];        
                $obj = json_decode($Data);
                $Print = UserModel::UpdateUserPassByAdmin($obj->{'USERID'}, $obj->{'newpass'});
				$logger->debug($Print); // to create debug type log file
                echo json_encode($Print);
                return;            
            break;
