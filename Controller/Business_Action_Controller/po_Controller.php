...
if($Type == null)
  ...
  // end
        ...
        while($k < sizeof($obj->{'_items'}))
        {
			...
			$itemlandingPrice = Managementapproval_Model::getItemLandingPrice($obj->_items[$k]->po_codePartNo,$obj->_items[$k]->po_principalId,'E');
			
			
            if($itemlandingPrice != 0){
				// Added on 28 July 2018 to make profit margin dynamic
				$PROFIT_MARGIN = Principal_Supplier_Master_Model::Get_principal_supplier_profitMargin($obj->_items[$k]->po_principalId);
				$itemlandingPrice = ($itemlandingPrice + (($itemlandingPrice * $PROFIT_MARGIN)/100));
				if($itemlandingPrice > $obj->_items[$k]->po_price){
					 $automatic_MA="Y";
					 $aRemarks=$aRemarks."Profit Margin is less than ".$PROFIT_MARGIN."% for Item -: ".$obj->_items[$k]->cPartNo."\n";
       ...
       //print_r($obj->{'_items'});exit;
        ...
        while($k < sizeof($obj->{'_items'}))
        {
            ...
            if($itemlandingPrice != 0){
				...
				if($itemlandingPrice > $obj->_items[$k]->po_price){
					 $automatic_MA="Y";
					 $aRemarks=$aRemarks."Profit Margin is less than ".$PROFIT_MARGIN."% "."\n";
