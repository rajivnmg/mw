...
switch($Type){
    ...
    case QueryModel::UPDATE:
        ...
        $Print = 0;
         //print_r($obj);exit;
         // commented on 28 july 2018 due to pf does not updated
        ...
        $Print = Outgoing_Invoice_Excise_Model::UpdateOutgoingInvoiceExciseNew($obj->{'oinvoice_exciseID'},$obj->{'oinvoice_No'}, $obj->{'poid'}, $obj->{'BuyerID'},$obj->{'principalID'},$obj->{'supplierID'},$obj->{'mode_delivery'},$obj->{'oinv_date'},$obj->{'oinv_time'},$obj->{'poid'},$obj->{'total_discount'},$obj->{'ed_tag'},$obj->{'freight_percent'},$obj->{'freight_amount'},$obj->{'pf_chrg'},$obj->{'incidental_chrg'},$obj->{'bill_value'},"P",$obj->{'remarks'},$USERID,$obj->{'ms'},$obj->{'ins_charge'},$obj->{'othc_charge'}, $obj->{'_supply_place'}, $obj->{'_reverse_charge_payable'}, $obj->{'_dnt_supply'}, $obj->{'_docket_no'},$obj->{'p_f_gst_rate'}, $obj->{'p_f_gst_amount'}, $obj->{'inc_gst_rate'}, $obj->{'inc_gst_amount'}, $obj->{'ins_gst_rate'}, $obj->{'ins_gst_amount'}, $obj->{'othc_gst_rate'}, $obj->{'othc_gst_amount'}, $obj->{'freight_gst_rate'}, $obj->{'freight_gst_amount'});
    ...
    case "GET_PO_PRINCIPAL":
        ...
        return;
        break;
    case "GET_PO_ITEM_PROFIT_MARGIN":
		
			$rsponce['message'] = "Landing Price Not Available";
			$rsponce['code'] = 3;
		
        $itemlandingPrice = Managementapproval_Model::getItemLandingPrice($_REQUEST['po_codePartNo'],$_REQUEST['po_principalId'],$_REQUEST['po_ed_applicability']);
        //$Print = Outgoing_Invoice_Excise_Model::LoadPrincipal($_REQUEST['PO_ID'],$_REQUEST['po_ed_applicability']);
		if($itemlandingPrice != 0){
				//function to get the item desc 
				$item_details = Managementapproval_Model::getItemDetails($_REQUEST['po_codePartNo']);
				
				// Added on 28 July 2018 to make profit margin dynamic
				$PROFIT_MARGIN = Principal_Supplier_Master_Model::Get_principal_supplier_profitMargin($_REQUEST['po_principalId']);
				$itemlandingPrice = ($itemlandingPrice + (($itemlandingPrice * $PROFIT_MARGIN)/100));
				if($itemlandingPrice > $_REQUEST['po_price']){					
					 $aRemarks.= "Profit Margin is less than ".$PROFIT_MARGIN."% for Item -: ".$item_details."\n";
					$rsponce['message'] = $aRemarks;
					$rsponce['code'] = 1;
					
				}else{
					$rsponce['message'] = "All Good";
					$rsponce['code'] = 2;
					
				}
			}	
		echo json_encode($rsponce);	
        return ;
        break;
    case "GETPODID":
    ...
    case "GET_INVOICE_LIST":
        $Print = Incoming_Inventory_Model::LoadIncoming_InventoryForBilling($_REQUEST['ITEMID'],"E",$_REQUEST['selectedItem']);
        $logger->debug(json_encode($Print));	// to create debug type log file
		echo json_encode($Print);
        return;
        break;        
    case "GET_ALL_INVOICE_LIST":
        $Print = Incoming_Inventory_Model::LoadIncoming_InventoryForBillingNEW($_REQUEST['ITEMID'],"E");
        $logger->debug(json_encode($Print));	// to create debug type log file
		echo json_encode($Print);
        return;
        break;
