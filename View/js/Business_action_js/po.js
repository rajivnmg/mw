    ...
    if (value != "" && data!="") {
        ...
        jQuery.ajax({
            ...
            success: function (jsondata) {
               // alert(jsondata);
                var objs = jQuery.parseJSON(jsondata);
                if(jsondata != "" && ((objs[0]._GSTList.length > 0) && (objs[0]._GSTList[0]._gst_no.length >0 ))){
					if (jsondata != "") {
			...
			//$("#bemailId").val(objs[0]._email);
					...
					alert("GST Detail is not added in this Buyer. Please Add GST Detail first in Buyer master");
