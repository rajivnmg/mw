...
Outgoing_Invoice_Excise_App.controller('Outgoing_Invoice_Excise_Controller', ['$scope', '$http', function Outgoing_Invoice_Excise_Controller($scope, $http) {
    ...
    $scope.addItem = function () {
		var profitMarginAdmin = false;
		var profitMargin = false;
		//console.log("TTTTTTTTTT",$scope.outgoing_invoice_excise)
        var issuedQty = parseFloat($scope.outgoing_invoice_excise.issued_qty);
        if (issuedQty > 0) {
			
			//condition to check the profit margin report added on 25 Aug,2018: 
			jQuery.ajax({
				url: URL,
				type: method,
				 cache: false,
				 async : false,
				data: { TYP: "GET_PO_ITEM_PROFIT_MARGIN",po_codePartNo:$scope.outgoing_invoice_excise._item_id,po_principalId:$("#principal_list").val(),po_price:$("#oinv_price").val(), po_ed_applicability:'E' },
				success: function (jsondata2) {
					var objs22 = jQuery.parseJSON(jsondata2);					
					 if((objs22['code'] == 1 || objs22['code'] == 3) && ($("#userType").val() =='M' || $("#userType").val() =='A') ){
						  if (confirm(objs22['message']+', Still You may continue')) {
								profitMarginAdmin = true;
								profitMargin = true;
							}else{
								return ;
							}
						 
					}else if((objs22['code'] == 1 || objs22['code'] == 3) && ($("#userType").val() !='M' || $("#userType").val() !='A')){
						profitMargin = false;
						alert(objs22['message']+', Only Admin OR Management can create this invoice')
						return ;
					}else{
						profitMarginAdmin = true;
						profitMargin = true;
			...
			if(profitMarginAdmin == true &&  profitMargin == true){
				//alert($("#itemid").val());
				//alert($("#invoice_list").val())
				//selectedItem
				/* alert('freight_percent --> ' + $("#txtfreight_percent").val());
				alert('freight_amount --> ' + $("#txtfreight_amount").val());
				alert('p_f_charge --> ' + $("#txtpf_charg").val());
				alert('inc_charge --> ' + $("#txtincidental_chrg").val());
				alert('ins_charge --> ' + $("#ins_charge").val());
				alert('othc_charge --> ' + $("#othc_charge").val()); */
				if((parseFloat($("#txtfreight_percent").val()) > 0 ) ||(parseFloat($("#txtfreight_amount").val()) > 0 ) ||(parseFloat($("#txtpf_charg").val()) > 0 ) ||(parseFloat($("#txtincidental_chrg").val()) > 0 ) ||(parseFloat($("#ins_charge").val()) > 0 ) ||(parseFloat($("#othc_charge").val()) > 0 ))
				{
					if($scope.outgoing_invoice_excise._items.length > 0)
					{
						...
						if($scope.outgoing_invoice_excise.igst_rate == 0)
						{
							new_gst = parseFloat($scope.outgoing_invoice_excise.cgst_rate) + parseFloat($scope.outgoing_invoice_excise.sgst_rate);
							new_gst = parseFloat(new_gst).toFixed(2);
						}
						else
						{
							new_gst = parseFloat($scope.outgoing_invoice_excise.igst_rate).toFixed(2);
						}
						//alert('new_gst ' + new_gst);
						//console.log("TTTTTT",$scope.outgoing_invoice_excise._items)
						var k = 0;
						while (k < $scope.outgoing_invoice_excise._items.length)
						{
							...
							//~ while (ic < k)
							//~ {							
								//commented on 28 july 2018 for adding same incoming invoice multipe tile
								//if($scope.outgoing_invoice_excise._items[k]["_item_id"] == $scope.outgoing_invoice_excise._items[ic]["_item_id"])
								
								// added on 28 july 2018 to prevent the adding same incoming invoice more then one time on same outgoing invoice
							/*	if($scope.outgoing_invoice_excise._items[k]["_item_id"] == $("#itemid").val())
								{
							...
							//alert($scope.outgoing_invoice_excise._items[k]["igst_rate"]);
							if($scope.outgoing_invoice_excise._items[k]["igst_rate"] == 0)
							{
								old_gst = parseFloat($scope.outgoing_invoice_excise._items[k]["cgst_rate"]) + parseFloat($scope.outgoing_invoice_excise._items[k]["sgst_rate"]);
								old_gst = parseFloat(old_gst).toFixed(2);
							}
							else
							{
								old_gst = parseFloat($scope.outgoing_invoice_excise._items[k]["igst_rate"]).toFixed(2);
							}
							//alert('old_gst ' + old_gst);
							if(new_gst !== old_gst)
							{
								alert('GST rates of all items must be same.');
								$scope.outgoing_invoice_excise.cgst_rate = "";
								$scope.outgoing_invoice_excise.discount = "";
								$scope.outgoing_invoice_excise.tot_price = "";
								$scope.outgoing_invoice_excise.igst_amt = "";
							...
							{
								$scope.outgoing_invoice_excise.p_f_gst_rate = new_gst;
								$scope.outgoing_invoice_excise.freight_gst_rate = new_gst;
								$scope.outgoing_invoice_excise.ins_gst_rate = new_gst;
								$scope.outgoing_invoice_excise.inc_gst_rate = new_gst;
								$scope.outgoing_invoice_excise.othc_gst_rate = new_gst;
							}
							k++;
						}
				...
				{
					$scope.outgoing_invoice_excise.p_f_gst_rate = 0;
					$scope.outgoing_invoice_excise.freight_gst_rate = 0;
					$scope.outgoing_invoice_excise.ins_gst_rate = 0;
					$scope.outgoing_invoice_excise.inc_gst_rate = 0;
					$scope.outgoing_invoice_excise.othc_gst_rate = 0;
				}
				$("#btnsave").hide();
				//alert($("#itemid").val());
				...
				if (discount_flag) {
					...
					selectedItem.push($("#invoice_list").val());
					console.log("selectedItem"+selectedItem);
				//console.log("UUUUUU",$scope.outgoing_invoice_excise._items)
				}
				else {
    ...
    $scope.BillCalculation = function () {
        ...
        while (k < $scope.outgoing_invoice_excise._items.length) {
          ...
          //  cvd_amt = cvd_amt + parseFloat($scope.outgoing_invoice_excise._items[k]["cvd_amt"]);
			...
			$scope.outgoing_invoice_excise.bill_value = parseFloat($scope.outgoing_invoice_excise.bill_value).toFixed(2);
		...
		PFGSTAmt = ((parseFloat(PFChargeAmt) * parseFloat($scope.outgoing_invoice_excise.p_f_gst_rate)/100));
        PFGSTAmt = parseFloat(PFGSTAmt).toFixed(2);
		//$scope.outgoing_invoice_excise.p_f_gst_rate = 18;
		$scope.outgoing_invoice_excise.p_f_gst_amount = PFGSTAmt;
		
		IncidentalGSTAmt = ((parseFloat(IncidentalChargeAmt) * parseFloat($scope.outgoing_invoice_excise.inc_gst_rate)/100));
        IncidentalGSTAmt = parseFloat(IncidentalGSTAmt).toFixed(2);
		//$scope.outgoing_invoice_excise.inc_gst_rate = 18;
		$scope.outgoing_invoice_excise.inc_gst_amount = IncidentalGSTAmt;
		
		InsuranceGSTAmt = ((parseFloat(InsuranceChargeAmt) * parseFloat($scope.outgoing_invoice_excise.ins_gst_rate)/100));
        InsuranceGSTAmt = parseFloat(InsuranceGSTAmt).toFixed(2);
		//$scope.outgoing_invoice_excise.ins_gst_rate = 18;
		$scope.outgoing_invoice_excise.ins_gst_amount = InsuranceGSTAmt;
		
		OtherGSTAmt = ((parseFloat(OtherChargeAmt) * parseFloat($scope.outgoing_invoice_excise.othc_gst_rate)/100));
        OtherGSTAmt = parseFloat(OtherGSTAmt).toFixed(2);
		//$scope.outgoing_invoice_excise.othc_gst_rate = 28;
		$scope.outgoing_invoice_excise.othc_gst_amount = OtherGSTAmt;
		
		FreightGSTAmt = ((parseFloat(FreightChargeAmt) * parseFloat($scope.outgoing_invoice_excise.freight_gst_rate)/100));
        FreightGSTAmt = parseFloat(FreightGSTAmt).toFixed(2);
		//$scope.outgoing_invoice_excise.freight_gst_rate = 18;
		$scope.outgoing_invoice_excise.freight_gst_amount = FreightGSTAmt;
		...
		$scope.outgoing_invoice_excise.bill_value = parseFloat($scope.outgoing_invoice_excise.bill_value) + parseFloat(PFChargeAmt) + parseFloat(IncidentalChargeAmt) + parseFloat(InsuranceChargeAmt) + parseFloat(OtherChargeAmt) + parseFloat(FreightChargeAmt) + parseFloat(PFGSTAmt) + parseFloat(IncidentalGSTAmt) + parseFloat(InsuranceGSTAmt) + parseFloat(OtherGSTAmt) + parseFloat(FreightGSTAmt);
		$scope.outgoing_invoice_excise.bill_value = parseFloat($scope.outgoing_invoice_excise.bill_value).toFixed(2);
...
//~ 
    ...
    $scope.removeItem = function (item) {
        ...
        $scope.BillCalculation();
        if($scope.outgoing_invoice_excise._items.length === 0){
			$scope.invoiceBillCalculation();
		}
       // alert($scope.outgoing_invoice_excise._items.length)
    }
	
	//function created on 27 August 2018 to reset all value
	
	$scope.invoiceBillCalculation = function () {
		var PFChargeAmt =0;
        var IncidentalChargeAmt =0;
        var InsuranceChargeAmt =0;
        var OtherChargeAmt =0;
        ...
        $scope.outgoing_invoice_excise.pf_chrg = pf_chrg; 
        
       
        $scope.outgoing_invoice_excise.incidental_chrg = incidental_chrg; 
        $scope.outgoing_invoice_excise.incidental_chrg_percent = incidental_chrg; 
      
        $scope.outgoing_invoice_excise.ins_charge = ins_charge; 
		
		$scope.outgoing_invoice_excise.othc_charge = othc_charge; 
		
        $scope.outgoing_invoice_excise.freight_amount = "0.00"; 
        $scope.outgoing_invoice_excise.frgt = $("#freightTag").val(); 
	
		$scope.outgoing_invoice_excise.freight_percent = 0.00;
		$scope.outgoing_invoice_excise.p_f_gst_amount = 0;		
		
		$scope.outgoing_invoice_excise.inc_gst_amount = 0;		
	
		$scope.outgoing_invoice_excise.ins_gst_amount = 0;
	
		$scope.outgoing_invoice_excise.othc_gst_amount = 0;		
	
		$scope.outgoing_invoice_excise.freight_gst_amount = 0;
		
       
		$scope.outgoing_invoice_excise.bill_value = 0.00;
   ...
   String.prototype.splice = function( idx, rem, s ) {
    ...
    $scope.AddOutgoingInvoiceExcise = function () {
        ...
        $scope.outgoing_invoice_excise.principalID = $("#principal_list").val();
        $scope.outgoing_invoice_excise.supplierID = $("#select3").val();
        $scope.outgoing_invoice_excise._principal_gstin = $("#principal_gstin").val();
        $scope.outgoing_invoice_excise._supplier_gstin = $("#supplier_gstin").val();
            ...
            success: function (jsondata) {
                $scope.$apply(function () {
                   ...
                   if(jsondata == "CREDIT_TIME_ERROR"){
					...
					}else if(jsondata == "MYSQL_ERROR"){
						alert("Not Saved, Some error occured in inserting items.");
						//$("#btnsave").show();
						$scope.outgoing_invoice_excise = null;
						location.href = 'new_Outgoing_Invoice_Excise.php?OutgoingInvoiceNonExciseNum=';
						return;
					}
					else{
						alert("Successfully Saved.");
                        ...
                        location.href = 'print_type.php?TYP=EXCISE&invoiceID='+jsondata;
                    }
                });
            }
        });
      ...
      if ($scope.outgoing_invoice_excise._item_id > 0) {
            ...
            if (true) {
                jQuery.ajax({
                    url: URL,
                    type: "POST",
                    data: { TYP: TYPE, ITEMID: $scope.outgoing_invoice_excise._item_id,selectedItem:selectedItem},
                    //cache: false,
                    success: function (jsondata) {
                        //alert(jsondata);
                        var objs = jQuery.parseJSON(jsondata);
  ...
  /*      if (true) {
    ...
    // Function created on 28 july 2018 to show all invoice on demand when click on check box
    $scope.showAllInvoice = function () {    	
     var bpoType=$("#bpoType").val();
     var passtag="E";
     if ($scope.outgoing_invoice_excise.bpod_Id > 0) {   
      if ($scope.outgoing_invoice_excise._item_id > 0) {
            var TYPE = "GET_ALL_INVOICE_LIST";
            if (true) {
                jQuery.ajax({
                    url: URL,
                    type: "POST",
