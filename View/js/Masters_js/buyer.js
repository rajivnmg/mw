...
$scope.buyer._principalname = $("#principalid option:selected").text();
    ...
    $scope.submitForm = function () {
       ...
       //alert($scope.buyer._GSTList.length);
		for(var i = 0; i <= $scope.buyer._GSTList.length; i++){
			//alert($scope.buyer._GSTList.length);
			if ($scope.buyer._GSTList.length <= 0) {
				alert("Atleast one GST Detail should be added in a Buyer grid before Add.");
				return;
			}
		}	
         $("#btnaddbuyer").hide();
