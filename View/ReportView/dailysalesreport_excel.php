...
echo 'SNo' . "\t" . 'Invoice No'."\t".'Invoice Date' . "\t" . 'Principal_Name' . "\t" .'BuyerName' . "\t" .'Taxable Amount'."\t" . 'CGST Amt'."\t" .'SGST Amt'."\t" .'IGST Amt'."\t" .'Fre. Amt'."\t" .'P&F Amt'."\t" .'Ins. Amt'."\t" .'Inc. Amt'."\t" .'Oth. Amt'."\t" .'CGST Comp'."\t" .'SGST Comp'."\t" .'IGST Comp'."\t" . 'Total Amount'."\t"."\n";
...
{			
	...
	if($eachResult['other_amt'] !== "Total : ")
	{
		echo    $eachResult['SN'] ."\t".
				...
				number_format((float)$eachResult["cgst_camt"], 2, '.', '')."\t" .
				number_format((float)$eachResult["sgst_camt"], 2, '.', '')."\t" .
				number_format((float)$eachResult["igst_camt"], 2, '.', '')."\t" .				
